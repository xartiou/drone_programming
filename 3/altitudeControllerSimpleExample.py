...
class VehicleSimpleDynamic:  # отвечает за динамику ЛА
    ...
    def rightParts(self, rotorsAngularVel):  # получает ускорение исходя из угловой скорости двигателей
        # входное значение для угловой скорости рассчитывается системой управления и является управляющим воздействием
        '''
...
class ControlSystem():  # класс системы управления
    def __init__(self, k_p, k_i, k_d, controlLimit):
        ...
        self.error = 0  # ошибка
        self.errorPast = 0  # ошибка на прошлом шаге
        self.integral = 0  # 0 - т.к. система еще не двигалась
...
class Simulator():  # класс симулятора полета
...
'''
k_p = 300  # коэффициент Пропорционального регулирования отвечает время достижения результата
k_i = 35  # коэффициент Интегрального регулирования отвечает за статическую ошибку
k_d = 180  # коэффициент Дифференциального регулирования отвечает за колебания системы
